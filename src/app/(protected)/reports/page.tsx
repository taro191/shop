...
export default async function ReportsPage({ searchParams }: PageProps<"/reports">) {
  ...
  const [rows, marginRatio, branches] = await Promise.all([
    getReportRows(user.storeId, period, branchId),
    getAverageMarginRatio(user.storeId),
    prisma.branch.findMany({ where: { storeId: user.storeId }, orderBy: [{ isMain: "desc" }, { createdAt: "asc" }] }),
  ]);
  ...
  return (
    <div className="pb-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 pt-4 sm:px-8">
        {branches.length > 1 ? (
          <FilterChips
            basePath="/reports"
            paramName="branch"
            options={["__all__", ...branches.map((b) => b.id)]}
            current={branchId ?? "__all__"}
            otherParams={{ period: period !== "daily" ? period : undefined }}
            labelFor={(o) => (o === "__all__" ? "ทุกสาขา" : (branches.find((b) => b.id === o)?.name ?? o))}
            allValue="__all__"
          />
        ) : (
          <span />
        )}
        <ReportExportBar period={period} branchId={branchId} />
      </div>
      ...
      <div id="report-print">
        ...
        <div className="px-5 pt-5 sm:px-8 print:hidden">
